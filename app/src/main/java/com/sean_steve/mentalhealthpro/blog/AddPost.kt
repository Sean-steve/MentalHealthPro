...
import com.sean_steve.mentalhealthpro.IllnessRecyclerView
 ...
 * Use the [AddBlogsFragment] factory method to
...
class AddPost : Fragment() {
    ...
    override fun onCreateView(
        ...
        with(image) {
            ...
            // After click on image we will be selecting an image
            this?.setOnClickListener(View.OnClickListener { showImagePicDialog() })
        }
        ...
        with(upload) {
            ...
            // After click on image we will be selecting an image
            with(image) { this?.setOnClickListener(View.OnClickListener { showImagePicDialog() }) }

            // Now we will upload out blog
            this?.setOnClickListener(View.OnClickListener {

                //title.getText()toString().trim{it<=' '}
                //same to description

                val titl =title.toString().trim { it <= ' ' }
                val description =des.toString().trim { it <= ' ' }

                // If empty set error
                if (TextUtils.isEmpty(titl)) {
                    with(title) { setError("Title Cant be empty") }
                ...
                if (TextUtils.isEmpty(description)) {
                    with(des) { setError("Description Cant be empty") }
    ...
    private fun pickFromCamera() {
        ...
        imageuri = requireActivity().contentResolver.insert(MediaStore.Images.Media.EXTERNAL_CONTENT_URI,
            contentValues)
            ...
            if (uriTask.isSuccessful) {
                ...
                databaseReference.child(timestamp).setValue(hashMap)
                    .addOnSuccessListener {
                        ...
                        //insert from which activity it will pickfrom;


                        startActivity(Intent(context, IllnessRecyclerView::class.java))
