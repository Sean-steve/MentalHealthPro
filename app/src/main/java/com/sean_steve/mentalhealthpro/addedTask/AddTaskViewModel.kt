...
import com.sean_steve.mentalhealthpro.Event
...
class AddTaskViewModel(application: Application) : AndroidViewModel(application) {
        ...
        viewModelScope.launch {
            tasksRepository.getTask(taskId).let { result ->
                if (result is Success<*>) {
                    onTaskLoaded(result.data as Tasks)
                } else {
                    onDataNotAvailable()
                }
            }
    ...
    private fun onTaskLoaded(task: Tasks) {
    ...
    fun saveTask() {
        ...
        if (Tasks(currentTitle, currentDescription).isEmpty) {
            _snackbarText.value = Event(R.string.empty_task_message)
            return
        }
        ...
        if (isNewTask || currentTaskId == null) {
            createTask(Tasks(currentTitle, currentDescription))
        } else {
            val task = Tasks(currentTitle, currentDescription, taskCompleted, currentTaskId)
            updateTask(task)
        }
    }
